...
namespace IMS.Controllers
{
    public class ORDERsController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Order_ID,ID")] ORDER oRDER)
        {
                    ...
                    if (Ids.Length > 0)
                    {
                        ...
                        for (int i = 0; i < Ids.Length; i++)
                        {
                            ...
                            if (product != null)
                            {
                                ...
                                if (productDetails != null)
                                {
                                    if (productDetails.Product_Avaliable >= Qty[i])
                                    {

                                    }
                                    else
                                    {
                                        return product.Product_Name + ":" + productDetails.Product_Avaliable;
                                    }
                                }
                                else
                                {
                        ...
                        order.ID = Convert.ToInt32(Session["id"].ToString()); 
                            ...
                            for (int i = 0; i < Ids.Length; i++)
                            {
                                ...
                                if (product != null)
                                {
                                    ...
                                    if (productDetails != null)
                                    {
                                        ...
                                        productDetails.Product_Avaliable -= Qty[i];
                                        productDetails.Product_Sold += Qty[i];
        ...
        public void GeneratePDF(int orderNo,string accountNo, string PaymentType, int IsPaid,string Ids,string Qty)
        {
            ...
            using (StringWriter sw = new StringWriter())
            {
                using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                {
                    ...
                    sb.Append(" </td></tr>");
                    sb.Append("<tr><td colspan = '2'></td></tr>");
                    sb.Append("<tr><td><b>Payment Type: </b>");
                    sb.Append(PaymentType);
                    sb.Append("</td><td align = 'right'><b>Is Paid: </b>");
                    sb.Append(IsPaid == 0 ? "Not Paid" : "Paid" );
                    sb.Append(" </td></tr>");
                    sb.Append("</table>");
                    sb.Append("<br />");
        ...
        public void GenerateInvoice(int orderNo)
        {
            DataTable dt = new DataTable();
            dt.Columns.AddRange(new DataColumn[4] {
                        new DataColumn("Product", typeof(string)),
            ...
            var orderDetails = db.ORDER_DETAIL.Where(od => od.Order_ID == orderNo).ToList();
            ...
            foreach (var orderDetail in orderDetails)
            {
                ...
                var total = orderDetail.Unit_Price  * orderDetail.Quantity;
                dt.Rows.Add(product.Product_Name, orderDetail.Unit_Price, orderDetail.Quantity, total);
            ...
            using (StringWriter sw = new StringWriter())
            {
                using (HtmlTextWriter hw = new HtmlTextWriter(sw))
                {
                    ...
                    sb.Append("<tr><td colspan = '2'></td></tr>");
                    sb.Append("<tr><td><b>Payment Type: </b>");
                    sb.Append(bill.Payment_Type);
                    sb.Append("</td><td align = 'right'><b>Is Paid: </b>");
                    sb.Append(bill.Is_Paid == true ? "Paid" : "Not Paid");
                    sb.Append(" </td></tr>");
                    sb.Append("</table>");
                    sb.Append("<br />");
